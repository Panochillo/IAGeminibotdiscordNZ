...
// Lista de baneados directamente aquí
let bannedUsers = ["1275173476126163079"]; // puedes empezar vacío []
...
client.on("messageCreate", (message) => {
  ...
  // Comando banbot
  if (message.content.startsWith("!banbot")) {
    ...
    if (bannedUsers.includes(user.id)) {
      return message.reply("Ese usuario ya está baneado del bot.");
    }

    bannedUsers.push(user.id);
    message.reply(`✅ ${user.tag} ha sido baneado del bot.`);
  }
});
