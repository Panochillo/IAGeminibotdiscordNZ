    ...
    async execute(interaction) {
        ...
        if (bannedUser) {
            if (config.BAN_SETTINGS.LOG_ATTEMPTS) {
                logger.warn(`Banned user ${interaction.user.username} (${userId}) attempted to use image command`);
            }
        ...
        if (!prompt || prompt.trim().length === 0) {
            const embed = new EmbedBuilder()
                ...
                .setDescription('¡Por favor proporciona una descripción válida para la imagen!')
                .setTimestamp();
